...
# New Logic-LM Solver
try:
    from z3_solver import Z3SyllogismSolver
except ImportError:
    Z3SyllogismSolver = None
...
class SymbolicSyllogismEngine:
    ...
    def __init__(self, gemini_model=None, t5_parser=None, use_existential_import: bool = True, use_z3: bool = True):
        ...
        self.z3_solver = None
        if use_z3 and Z3SyllogismSolver:
            print("Initializing Logic-LM Z3 Solver...")
            self.z3_solver = Z3SyllogismSolver(check_install=False)
    ...
    def predict_validity(self, syllogism: str, use_fallback: bool = True, anonymize: bool = False) -> Tuple[bool, Dict]:
        ...
        # STRATEGY 1: Logic-LM (Z3 Solver) - PRIMARY
        if self.z3_solver and self.gemini_model:
            # We pass the ORIGINAL syllogism to Z3 (it handles semantics better)
            # OR anonymized? Logic-LM usually works on raw text or slightly normalized.
            # Let's use raw syllogism for Z3 as it can define 'Socrates' as a constant.
            
            z3_res = self.z3_solver.prove_validity(self.gemini_model, syllogism)
            
            if z3_res.get("z3_result") in ["VALID", "INVALID"]:
                 return z3_res["validity"], {
                     "method": "Z3_LogicLM",
                     "z3_result": z3_res["z3_result"],
                     "code": z3_res.get("generated_code")
                 }
            # If "ERROR", fall back to symbolic extraction
        
        # STRATEGY 2: Symbolic Parser (Fallback)
        # Check cache
